...
"""
print "Loading File from UniProt..."
...
session.run("MATCH (p:Protein) WHERE p.chembl_id = '' REMOVE p.chembl_id")

prot_info = session.run("MATCH (p:Protein) return p.name as name, p.identifier as id, p.description as desc, p.reviewed as rev, keys(p) as keys, p.chembl_id as chembl")
prot_dict = dict()
arr_exists = []
iso_exists = []

print "Deleting previous Isoform Property Keys ..."
for rec in prot_info:
    ...
    if does_isokey_exist(keys_arr):
        iso_arr = extract_iso(keys_arr)
    else:
        iso_arr = []
        
    single_prot_dict = {"identifier":id_prot, "name":name_prot, "iso_array":iso_arr, "desc":desc_prot, "rev":rev_prot, "chembl":chembl_prot}
    prot_dict.update({id_prot:single_prot_dict})
    
    init_string = "MATCH (p:Protein) WHERE p.identifier = '" + id_prot + "' REMOVE " 
    if (len(iso_arr) > 0):
        iso_exists.append(id_prot)
        for n in range(len(iso_arr)):
            if (n == 0):
                tmp = init_string
                n_iso = "p." + iso_arr[n]
                init_string = tmp + n_iso
            else:
                tmp = init_string
                n_iso = ", p." + iso_arr[n]
                init_string = tmp + n_iso
        session.run(init_string) # Deleting all of the Isoform Property Keys that currently exist [ONLY PART OF THIS VERSION OF THE UPDATE SCRIPTS]

print "All isoform keys deleted!"
time.sleep(2)
...
for it1, rw1 in prot_df.iterrows():
    ...
    if not (str(rw1["Entry"])) in add_arr:
        prev_dict = {"prot_id":prot_dict[str(rw1["Entry"])]["identifier"], "prot_name":prot_dict[str(rw1["Entry"])]["name"], "prot_desc":prot_dict[str(rw1["Entry"])]["desc"], "prot_rev":prot_dict[str(rw1["Entry"])]["rev"], "prot_chembl":prot_dict[str(rw1["Entry"])]["chembl"]}
        current_dict = {"prot_id":rw1["Entry"], "prot_name":rw1["Entry name"], "prot_desc":rw1["Protein names"], "prot_rev":prot_rev, "prot_chembl":prot_chembl}
        if not str(rw1["Alternative products (isoforms)"]) == "nan":
            cnt_iso_update = cnt_iso_update + 1
            iso = create_iso_key(process_iso(str(rw1["Alternative products (isoforms)"])), db, str(rw1["Gene names"]), str(rw1["Protein names"]), str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Organism"]).replace(" (Human)", ""), pe_converter(str(rw1["Protein existence"])), str(rw1["Version (sequence)"]))

            if not (prev_dict == current_dict):
                cnt_update = cnt_update + 1
                session.run(update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, iso, prot_chembl))
                #print update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, iso, prot_chembl)
            else:
                session.run(update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, iso, prot_chembl))
                #print update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, iso, prot_chembl)
        else:
            if not (prev_dict == current_dict):
                cnt_update = cnt_update + 1
                session.run(update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, "None", prot_chembl))
                #print update_prot_neo4j(str(rw1["Entry"]), str(rw1["Entry name"]), str(rw1["Protein names"]), prot_rev, "None", prot_chembl)
print "Data Updated! Look below for stats of this update:"

print "Number Updated : " + str(cnt_update)
print "Number of Isoforms Updated : " + str(cnt_iso_update)
